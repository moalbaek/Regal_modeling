...
**Purpose:** The v1 scenario model calibrates assumed pooled survival families to the publicly disclosed
...
**Deliverables:** `regal_explorer.html` (self-contained tabbed interface that lands on the interactive
V1 explorer and provides the V2 result view in a second tab), `regal_explorer.py` (the v1 engine in Python, with additional
...
> **V1 scenario status.** The current `ps` values are fixed-scenario rejection rates conditional on the selected assumptions
> and on reaching the final analysis. V1 does not condition on the observed decision to continue
> after the 60-event interim. The browser lands on the V1 tab. The separate V2 tab reads an embedded
> result bundle generated by Python and displays no headline unless the complete model average clears
> every release gate. See
> [`V2_IMPLEMENTATION_PLAN.md`](V2_IMPLEMENTATION_PLAN.md) for the rebuild status.

**Last updated:** 2026-09-04 · **Status:** research/analysis tool, not investment advice.
...
The v1 survival, fitting, and final-analysis engine is delivered in two forms:

| File | Role | Key outputs |
|------|------|-------------|
| `regal_explorer.html` | Self-contained interface with an embedded Python v2 bundle and a separately labeled v1 explorer. | readiness-gated v2 headline/sensitivities/diagnostics; v1 fixed-scenario rates and interactive charts |
...
| `trial_design.py` | V1 classical audit boundary plus isolated v2 Lan-DeMets spending and protocol-factor stratified score analysis. | v1 and v2 efficacy boundaries; stratified primary statistic; named one-step diagnostics |
...
| `simulation.py` | Isolated v2 event-driven decision process; not consumed by the v1 explorer. | 60/80-event cutoffs, efficacy/futility/continuation branches, final decisions, canonical operating characteristics |
...
| `survival_models.py` | Corrected, isolated v2 survival primitives; not consumed by the v1 explorer. | scale-aware OS/net cure mixtures, population mortality, pre-outcome frailty/case mix, post-selection randomization |
| `bat_regimens.py` | Corrected, isolated v2 BAT representation; not consumed by the v1 explorer. | joint planned-stratum/delivered-regimen pathways, combination exposure, one survival profile per patient, and scenario-role labels |
...
| `event_likelihood.py` | Isolated v2 public-history likelihood; not consumed by the v1 explorer. | registry-anchored fixed-N accrual, correlated Poisson-multinomial event increments, event-80 announcement right censor |
...
| `posterior.py` | Isolated WP6/WP7 conditioning and model-averaging engine; not consumed by the v1 explorer. | consistent latent histories, exact public-count conditional draws, continuation importance weights, seven GPS effect families, parameter priors, posterior family weights, prior sensitivity, ESS diagnostics |
...
The two v1 implementations share one enrollment reconstruction, one set of survival primitives,
...
reuse the v1 accrual or fitting path:
...
| **V1 significance threshold** | observed one-step HR ≤ **0.636**, i.e. z_crit = \|ln 0.636\|·√80 / 2 = **2.024** (one-sided p ≈ 0.0215) | [D] | Preserved for v1 numerical reproducibility; matches SELLAS's stated design effect size but is not the v2 boundary implementation. |
...
| **V1 code reconstruction (base)** | slow 2020–21 → heavy 2022–23 → China bolus, summing to 126; implied median ≈ Mar 2023 | [A] | Preserved for v1 reproducibility, but starts before study opening and misses the first-20 anchor. |
...
**V1 scenario presets** (selectable in the explorer; weights auto-normalize):
...
(40/10/25/15/10, ven cure 10%) — the bear corner is the only scenario composition that pushes the
...
weights are retained only as a historical comparison. The primary's single-profile delivered-regimen
...
25%. Keeping allocation and survival parameters separate makes the historical bear assumption fully
...
≈ 9%, BAT median ≈ 8.1 mo before enrollment selection (≈ 12% / ≈ 10.7 mo after the evidence-informed
selection defaults and the 1–6-month entry window). Weights follow the US/EU/China BAT-composition review (observation ~35%, venetoclax ~35%,
...
(`esel`, 0–50%, default 20%) makes that gap an explicit lever.

| Parameter | Value | Type | Source / reasoning |
|-----------|-------|------|--------------------|
| Eligibility screen-out `q` | 0–50%, default 20% | [A] | Evidence-informed estimate of the **frailty-linked** fraction rejected from the source population. It is deliberately below published all-cause AML trial-ineligibility rates because mutation mismatch, consent, logistics, treatment choice, and timing failures must not be treated as prognostic selection [R14][R15]. Screening acts on **baseline frailty**, not realized survival, so this is a probabilistic tilt (acceptance ∝ `exp(−β Z)`). Use 10–30% for sensitivity analysis. |
| Population frailty variance `θ` | 0–3, default 0.35 | [A] | Central working assumption informed by the large survival separation among AML first-relapse risk groups [R12] and by MRD/cytogenetic/time-to-transplant heterogeneity in 1,042 CR2 patients [R13]. Neither cohort directly estimates gamma frailty in REGAL's transplant-ineligible population, so use 0.20–0.60 for sensitivity analysis. |
...
**Evidence translation and limits.** In the 599-patient European Prognostic Index validation,
favorable/intermediate/poor first-relapse groups represented 13%/23%/64% of patients and had
one-year OS of 64%/38%/17% [R12]. Treating those one-year probabilities as exponential only for a
scale calculation gives a between-group relative-hazard variance of about 0.125. In the EBMT CR2
cohort, 28% were MRD-positive; two-year relapse was 40% versus 24% for MRD-negative patients, and
cytogenetics and time from diagnosis to transplant were also prognostic [R13]. A similar binary
calculation gives about 0.10. These are contextual lower bounds rather than estimates of `θ`: they
average away within-group variation and describe younger first-relapse or transplant-selected CR2
populations rather than REGAL's transplant-ineligible cohort. The default `θ = 0.35` allows for that
unrepresented spread without placing the base case at the high end of the sensitivity range.

The public REGAL record reports enrollment but no screened denominator. Across 13 AML registration
trials, published ineligibility was 26.7–35.9% by reported race, with disease-mutation mismatch a
material reason [R14]. A post-transplant AML/MDS maintenance trial reported 41% failing eligibility,
principally because of comorbidity and cytopenias, but its population and criteria were materially
different from REGAL [R15]. Because `q` in this model represents only selection correlated with
frailty—and the entry-window layer separately handles deaths before randomization—the default is
20%, not a direct copy of either raw screen-failure rate.

At the evidence-informed defaults, the enrolled cohort's mean frailty multiplier is
`(1−q)^θ = 0.8^0.35 = 0.925`, corresponding to a 7.5% lower mean uncured disease hazard among those
...
**Criterion-specific corroboration: platelet and lymphocyte recovery.** REGAL requires platelets
`>20 × 10^9/L` and an absolute lymphocyte count (ALC) `>0.3 × 10^9/L` [R1]. Those floors make the
direction of frailty-linked enrichment clinically plausible, but they do not separately identify
`q` or `θ`. In the most directly relevant platelet study, a pre-allogeneic-transplant cohort in
second or later remission had 36-month OS of 58% with complete platelet recovery versus 31% in
CR2p, with an OS HR of 1.9 for CR2p [R16]. That is an upper-bound signal rather than a REGAL effect:
CRp meant platelets below `100 × 10^9/L`, not failure of REGAL's much lower
`20 × 10^9/L` floor, and every patient proceeded to transplant. A separate pretransplant cohort
found no material survival difference among CR, CRp, and morphologic leukemia-free state, reinforcing
the setting dependence [R17].
...
HR 3.7 [R19]. Neither study represents adult, non-transplant CR2 maintenance or a single enrollment
measurement, so neither effect size should be imported into the model.

As an explicitly analyst-derived translation [A], the platelet and ALC screens together support
testing a 0–15% reduction in mortality hazard versus a broad, otherwise comparable CR2 population,
with 7% as the central case. The central multiplier of `0.93` is nearly identical to v1's default
combined frailty multiplier of `0.925`. It therefore **corroborates the existing default rather than
creating an additional adjustment**: multiplying another `0.93` on top would double-count the same
marrow/immune-recovery signal. Platelet count, ALC, ANC, treatment recovery, performance status, and
organ function share causes and prognostic information; where a source survival cohort already
applied similar count-recovery criteria, the incremental multiplier should be moved toward 1.00.
...
each of the five modeled BAT components (spanning the protocol's four BAT options because Observation
and palliative Hydroxyurea are represented separately), and the GPS non-responders who track
Observation through `Ssel`.
...
> modelled enrichment, with any shortfall left to sensitivity analysis around `q` and `θ` — on the grounds
...
- **Its direction is not guaranteed, and depends on the baseline hazard slope.** Only the
  ...
  of the configured cure/frailty mixture, not a property of delayed entry. On the real components, delayed entry alone
...
apply to the components whose medians are CR2-clock **population** quantities: all five modeled BAT
components spanning the four protocol options, and the GPS non-responders who track Observation.
Neither layer applies to the **fitted** GPS responder families — the plateau panel's `π_resp` and the
bounded alternative's `m_G`/`s_G` — for
...
The evidence-informed default is **`q = 20%`**, with **10–30%** used as the primary sensitivity band.

**Effect (base preset).** As `q` rises 0 → 20 → 50% (at the default `θ = 0.35` and the 1–6 mo entry
window) the BAT median OS lifts from 10.0 to 10.7 to 12.3 months. The BAT cure fraction barely moves (~12% throughout):
...
downward as selection increases, so the plateau scenario rate also falls — a healthier,
...
| Control | Range / default | Type | Role |
|---------|-----------------|------|------|
| Natural death rate | 0–10%/yr, default 2% | [A] | V1 overlays this competing risk on every component. V2 applies it according to whether an input is OS or net survival. |

**V1 mechanics.** The annual fraction `p` is converted to a constant monthly hazard
...
**V1 sensitivity.** In the reproduced base run, changing the v1 overlay from 0% to 2% moves
...
**Effect.** Dropout lowers the scenario rejection rate and can stall the trigger: at the evidence-informed
base preset the plateau rate falls ~97% → 96% → 93% → 75% across 0 / 3 / 5 / 10 %/yr, and the
80th-event "reached" fraction falls to ~84% at 10%. It is non-differential, so it dilutes the contrast
...
counts (some of those deaths are now "hidden" by dropout) — the GPS median moves ~88 → 44 → 28 mo
...
|---------|---------|------|------|
| Milestone weighting | weighted 1 / 2 / 4 (toggle to equal 1 / 1 / 1) | [A] | The default up-weights the most recent (most informative) milestone; the **unweighted** toggle treats 60/72/78 equally, testing whether the weighting choice drives the answer. At the evidence-informed selection default, equal weights move the GPS median ~88 → ~103 mo (`π_resp` 0.70 → 0.72), so the choice is materially load-bearing. |
| GPS-median Poisson interval | reported, not set | [D] | The event counts carry Poisson sampling noise, so the explorer refits at each count ±√n and reports the resulting **~68% interval on the derived GPS median** (about 22–231 mo at base). Its width shows how weakly three counts constrain the tail. |
...
enrollment selection q = 20%, frailty variance θ = 0.35, 0% loss-to-follow-up, weighted fit); every number is a function of the user controls in Sections
...
| BAT cure / median | ~12% · **10.7 mo** at the evidence-informed default; at θ = 0.35, the 1–6 mo entry window plus q=0 → 20% → 50% screening moves the median 10.0 → 10.7 → 12.3 mo. The cured fraction moves only through the entry window and edges down 12.16% → 11.92% → 11.48% as screening makes survival through the window less selective (Section 2.5.1). | `build_plateau` |
| GPS cure / median | ~57% · ~88 mo all-cause (disease-only plateau is never reached); both fall as selection rises and `π_resp` re-fits down | `build_plateau` |
| GPS median Poisson 68% CI | ~22–231 mo from 60/72/78 ±√n; wide because three pooled counts barely pin the tail | `fit_ci` |
| Pooled long-term-survivor fraction | ~0.34 (disease plateau; all-cause survival decays below it) | `build_plateau` |
| Pooled median OS | **~19 mo** (above the ≥13.5 floor) | `build_plateau` |
| Implied HR at the 60-event interim | ~0.45 (clears the 1.00 futility threshold); drifts toward 1 as selection rises | `mc` |
| Patients alive at the 80th event | ~34 GPS / ~12 BAT (before censoring) | `mc` |
| **Plateau fixed-scenario rejection rate** | **~97% at the evidence-informed default** at the committed 400-draw golden-snapshot budget (± about 2 pp Monte-Carlo error) | `build_plateau` + `mc` |
| **Bounded no-cure diagnostic** | At base the bounded no-cure fit is **State A (boundary/non-identified)**: responder median ≈64 mo, tail shape at the 1.50 light-edge boundary, milestone residual RMS ≈1.0, and m<sub>G</sub>/BAT ratio ≈6.0×. No scenario rejection rate is reported for a boundary fit. This is a parameter-box diagnostic, not a test that can prove or reject cure. | `build_no_gps_cure` + `mc` |
...
In the v1 implementation, the 2% natural-death default lifts the plateau reach fraction from
...
Sweeping the v1 BAT composition illustrates sensitivity. At the evidence-informed selection
default, the five-preset plateau rates are 100% (bull) · 100% (low) · 97% (base) · 67% (dom) ·
**18% (bear)** at the committed 400-draw golden-snapshot budget. The dom and bear presets land on an
adequate State-C alternative fit, with their own rates at approximately 76% and 24%; base, low, and
bull drive the bounded family to the light-edge parameter boundary (State A), so no alternative
rate is reported for those presets. The bear corner therefore remains capable of reversing the
base conclusion, but less strongly than under the previous selection assumptions. Other settings
can produce a residual-misfit status; all A/B/C labels remain conditional model-fit diagnostics.

> In the plateau scenario, the modeled disease dead fraction is now ~66%, modestly above the
...
`[1, 1, 1]` to test the choice; at base it shifts the GPS median by ~15 mo, ~88 → ~103, so the choice
...
The v1 `ps` output is the fraction of simulated trials whose final test is significant
...
- **State A — boundary / non-identified.** At least one fitted parameter reaches the box boundary.
  The compatibility `cure_req` field distinguishes upper/heavy from light-edge boundary subtypes for
...
- **State B — residual misfit.** The best interior fit exceeds the v1 RMS or maximum-residual
...
- **State C — adequate interior fit.** The bounded family matches the milestones within the v1
...
the primary default remains the protocol-compatible equal-planned-strata proxy, while the comparison and
...
nonzero if that bundle remains withheld. The HTML formats the bundle only; all v1 JavaScript
...
`buildNoGPSCure`, and the shared `Sweib`/`sampWeib`/`wscale` primitives). Their v1 survival,
...
4. **Under the plateau model, the scenario rejection rate is governed by the BAT-quality assumption.** With a
   ...
   range — the "bear corner" (70% venetoclax at a 25% cure) — lowers it to **~18%**. Removing
...
5. **Structural refinements are absorbed by the pooled-family fit.** Component-mixture BAT and a
   0–40% non-responder subgroup each leave the v1 scenario rate approximately unchanged because
   fitted parameters redistribute the assumed pooled trajectory. This localizes assumptions rather
   than identifying the arms.
6. **The bounded alternative is a sensitivity diagnostic.** At the base preset it reaches the
   light-edge parameter boundary (State A: median m<sub>G</sub> ≈64 mo, shape s<sub>G</sub> = 1.50),
   as do the low and bull presets, so no rejection rate is reported for them. The dom and bear
   presets retain adequate interior fits (State C). None of those statuses determines whether the
...
- **Enrollment selection screens on frailty, but the parameters remain analyst choices.** The
  eligibility lever (Section 2.5.1) selects on baseline frailty rather than realized survival, so it
  does not assume a screening accuracy no real criterion has. The evidence-informed defaults are
  `θ = 0.35` and `q = 20%`, based on the contextual AML/CR2 heterogeneity and screen-failure evidence
  in [R12]–[R15]. Those studies do not directly estimate either parameter in REGAL's population.
  `θ`, `q`, and the window length are only weakly separately identified from three aggregate event
  counts, so report the combined mean-frailty multiplier `(1−q)^θ` and treat 0.20–0.60 for `θ` and
  10–30% for `q` as sensitivity ranges rather than confidence intervals.
...
- **The plateau panel's GPS median is an unstable readout near `p_GPS = 0.5`.** It is the crossing point
  ...
  off to very large values and is hypersensitive to small parameter changes (dom `p_GPS` = 0.462 gives
  38 mo; base `p_GPS` = 0.569 gives 88 mo). Read the cure fraction and the rejection rate, not the
...
- **[R12]** Giles et al., “Validation of the European Prognostic Index for younger adult patients
  with acute myeloid leukaemia in first relapse,” *British Journal of Haematology* 2006;134:58–60.
  The 599-patient validation reports the size and one-year/five-year OS of three prognostic strata.
  https://pubmed.ncbi.nlm.nih.gov/16803568/
- **[R13]** Gilleece et al., “Measurable residual disease status and outcome of transplant in acute
  myeloid leukemia in second complete remission,” *Blood Cancer Journal* 2021;11:88. The
  1,042-patient CR2 cohort reports MRD prevalence, relapse, leukemia-free survival, cytogenetics, and
  timing associations. https://doi.org/10.1038/s41408-021-00479-3
- **[R14]** Pulte et al., “FDA Analysis of Ineligibility for Acute Myeloid Leukemia Clinical Trials
  by Race and Ethnicity,” *Clinical Lymphoma, Myeloma & Leukemia* 2023;23:463–470.e1. Reports
  ineligibility across 13 AML registration trials and identifies disease-mutation mismatch as one
  material reason. https://doi.org/10.1016/j.clml.2023.03.012
- **[R15]** Oran et al., “A phase 3 randomized study of 5-azacitidine maintenance vs observation
...
- **[R17]** Rashidi et al., “Hematologic recovery after pretransplant chemotherapy does not influence
  ...
  *Biology of Blood and Marrow Transplantation* 2015;21:1425–1430. Provides platelet-recovery
  counterevidence in another transplant cohort. https://pubmed.ncbi.nlm.nih.gov/25840336/
